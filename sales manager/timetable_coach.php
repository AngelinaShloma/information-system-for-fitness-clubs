...
<?php
            $type = mysqli_query($connect, "SELECT time_format(`time_start`, '%H-%i') FROM timetable_coach WHERE date = '$today' AND coach_id = '$coach_id'");
            $type = mysqli_fetch_all($type);
            foreach ($type as $t) {
            if ($t[0] >= $time) {
                ?>
    <a href="add_person_to_class.php?id=<?= $coach_id ?>&date=<?= $today ?>&time=<?= $t[0] ?>" class="time-btn"><?= $t[0] ?></a>
    <?php
            }else{
    ?>
    <a href="" class="time-late-btn"><?= $t[0] ?></a>
    <?php
            }
            }
    ?>
